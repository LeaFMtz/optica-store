<div class="contents">
    <div
        class="bg-[#71C229] text-white text-xs py-1.5 px-6 flex justify-end items-center space-x-2 w-full relative z-[60]">
        @auth
        ...
        <div class="px-4 py-4 mx-auto max-w-screen-2xl sm:px-6 lg:px-8 w-full">
            <div class="flex items-center justify-between gap-4">
                ...
                <div class="flex-1 flex justify-start lg:justify-center">
                    <a class="block hover:opacity-80 transition" href="{{ url('/') }}" wire:navigate>
                        <span class="sr-only">Home</span>
                        <img src="{{ asset('images/logo.webp') }}" alt="Óptica Guzmán"
                            class="h-10 w-auto lg:h-16 mx-auto">
                    </a>
                </div>
                    ...
                    <div x-data="{ mobileMenu: false }" class="lg:hidden">
                        ...
                        <div x-cloak x-transition x-show="mobileMenu"
                            ...
                            <ul x-on:click.away="mobileMenu = false"
                                ...
                                @auth
                                <li><span class="text-sm font-medium text-gray-400">Hola, {{ auth()->user()->name
                                        }}</span></li>
                                <li>
                                    <form method="POST" action="{{ route('logout') }}">
                                        @csrf
                                        <button type="submit"
                                            class="text-sm font-medium hover:text-[#71C229] transition">Cerrar
                                            sesión</button>
                                    </form>
                                </li>
                                @else
                                <li><a href="{{ route('login') }}" wire:navigate
                                        class="text-sm font-medium hover:text-[#71C229] transition">Iniciar sesión</a>
                                </li>
                                <li><a href="{{ route('register') }}" wire:navigate
                                        class="text-sm font-medium hover:text-[#71C229] transition">Crear cuenta</a>
                                </li>
                                @endauth
