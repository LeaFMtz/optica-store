<x-filament-widgets::widget>
  ...
  @php
    $childOptions = collect($this->configuredOptions)
        ->filter(fn ($opt) => !empty($opt['parent_key']))
        ->keyBy(fn ($opt) => $opt['parent_key']);
    $childIndices = collect($this->configuredOptions)
        ->map(fn ($opt, $idx) => ['idx' => $idx, 'opt' => $opt])
        ->filter(fn ($item) => !empty($item['opt']['parent_key']))
        ->keyBy(fn ($item) => $item['opt']['parent_key']);
  @endphp
  ...
  <div class="space-y-4">
    <div
      class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:divide-white/10 dark:bg-gray-900 dark:ring-white/10">
          ...
          <tbody class="divide-y divide-gray-200 whitespace-nowrap dark:divide-white/5">
            @foreach($this->configuredOptions as $option)
              @if(!empty($option['parent_key']))
                {{-- Task 4.3: child options are shown nested under parent — skip standalone row --}}
                @continue
              @endif
              <tr class="fi-ta-row">
                <td class="fi-ta-cell">
                ...
                <td class="fi-ta-cell">
                  <div class="fi-ta-text grid w-full gap-y-1 px-3 py-4">
                    ...
                    {{-- If this option has a child, show the valid combinations --}}
                    @if(!empty($option['is_parent']) && $childOptions->has($option['key']))
                      @php $child = $childOptions->get($option['key']); @endphp
                      <div class="mt-2 text-xs text-gray-500 dark:text-gray-400 space-y-1">
                        <span class="font-semibold">{{ $child['value'] }}:</span>
                        @foreach(($child['parent_value_selections'] ?? []) as $parentVal => $childVals)
                          <div class="ml-2">
                            <span class="text-gray-700 dark:text-gray-300">{{ $parentVal }} →</span>
                            {{ implode(', ', $childVals) }}
                          </div>
                        @endforeach
                      </div>
        ...
        <div class="fi-ta-empty-state">
          <div class="fi-ta-empty-state-content">
            ...
            <h4 class="fi-ta-empty-state-heading">{{
              __('lunarpanel::productoption.widgets.product-options.variants-table.empty.heading') }}</h4>
          </div>
        </div>
        @endif
      </div>
    ...
    <div wire:key="product_options">
      <div class="fi-ta-empty-state">
        <div class="fi-ta-empty-state-content">
          ...
          <h4 class="fi-ta-empty-state-heading">{{
            __('lunarpanel::productoption.widgets.product-options.options-list.empty.heading') }}</h4>
          <p class="fi-ta-empty-state-description">{{
            __('lunarpanel::productoption.widgets.product-options.options-list.empty.description') }}</p>
        </div>
      </div>
    </div>
    @else
    {{-- Task 4.1: product-options-list receives all items; child entries are skipped inside the component --}}
    <div>
      <x-lunarpanel::products.variants.product-options-list :items="$configuredOptions" group="product_options"
        state-path="configuredOptions" />
    </div>

    {{-- Task 4.2: for each parent option, render the child selections as editable checkboxes --}}
    @foreach($this->configuredOptions as $optionIdx => $option)
      @if(empty($option['is_parent']) || !$childIndices->has($option['key']))
        @continue
      @endif
      @php
        $childData = $childIndices->get($option['key']);
        $childIdx = $childData['idx'];
        $childEntry = $childData['opt'];
      @endphp
      <div wire:key="child_selections_{{ $optionIdx }}"
           class="overflow-hidden rounded-xl bg-blue-50 dark:bg-blue-950/20 ring-1 ring-blue-200 dark:ring-blue-800 p-4 space-y-3">
        <h4 class="text-sm font-semibold text-blue-900 dark:text-blue-200">
          {{ $childEntry['value'] }}
          <span class="font-normal text-blue-600 dark:text-blue-400 ml-1">(variantes vinculadas a {{ $option['value'] }})</span>
        </h4>
        <p class="text-xs text-blue-700 dark:text-blue-300">
          Seleccioná qué valores de <strong>{{ $childEntry['value'] }}</strong> aplican para cada valor de <strong>{{ $option['value'] }}</strong>.
        </p>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          @foreach($option['option_values'] as $parentValue)
            @if(!$parentValue['enabled'])
              @continue
            @endif
            <div class="space-y-1">
              <label class="fi-fo-field-label">
                <span class="fi-fo-field-label-content text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ $parentValue['value'] }}
                </span>
              </label>
              <div class="space-y-1 pl-2">
                @foreach($childEntry['option_values'] as $childValue)
                  <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                    <input
                      type="checkbox"
                      wire:model="configuredOptions.{{ $childIdx }}.parent_value_selections.{{ $parentValue['value'] }}"
